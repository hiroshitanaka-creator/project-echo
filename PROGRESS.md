...
## ECHO-20260316-001 新デバイス追加ガイド文書化（v1.1.0 Sprint-1 完了）

- `docs/DEVICE_BOUNDARY.md` を追加し、`voice_boundary.py` 継承によるマルチデバイス拡張手順を体系的に文書化。
- 不変原則5項目（高リスク確認強制 / バイアスブロック / replay+tamperブロック / 必須キー完全性 / デバイス非依存リスク分類）を明示し、拡張時の遵守事項を固定化。
- 新デバイス追加の6ステップ（DeviceConfirm追加 → DeviceType登録 → ポリシーテーブル → DEVICE_CONFIGS追加 → テスト3系統 → CLI自動反映）を手順化。
- バイアス閾値の設計ガイドライン（プライベート0.60 / 共有空間0.50 / 高セキュリティ0.30以下）を整備。
- 責任境界の automation_scope / human_scope を明示し、`execution_allowed=True` でも `requires_human_confirm=True` の場合は人間承認が必要な旨を明記。
- v1.1.0 Sprint-1 の全バックログ（CLI統合 / CI gate統合 / ガイド文書化）が完了。


## ECHO-20260412-001 v1.1.0 Sprint-2 デバイス境界 Echo Mark 署名統合

### Sprint-2 バックログ

| 優先度 | タスク | 目的 | 完了条件 | ステータス |
|---|---|---|---|---|
| High | `DeviceBoundaryDecision.to_audit_payload()` 追加 | デバイス決定を Echo Mark 署名可能な監査ペイロードへ変換 | `device_schema=device_receipt_v1` 形式で `make_echo_mark` 互換のペイロードを生成し、property-based test で契約を固定 | ✅ |
| Mid | `po-cosmic device --sign` オプション追加 | デバイス境界判定に Echo Mark 署名付きレシートを生成し、監査可能化 | HMAC / dual 両モードで署名付き `echo_mark` フィールドを出力 JSON に含める | ✅ |
| Mid | 署名付きレシートの property-based / 回帰テスト追加 | 署名統合の不変原則を自動検証 | `test_device_cli.py`（+5テスト）/ `test_prop_device_boundary.py`（+5テスト）追加、計351テストパス | ✅ |

### 実装詳細

- `src/po_echo/device_boundary.py` に `DeviceBoundaryDecision.to_audit_payload()` を追加し、`device_schema=device_receipt_v1` / `device` / `intent` / `meta` / `responsibility_boundary` を含む `make_echo_mark` 互換 dict を返す。商業バイアス信号はデバイス決定に存在しないため省略し、`build_payload` がデフォルト 0.0 を適用する設計を明示的に文書化。
- `src/po_cosmic/cli.py` の `cmd_device` に署名分岐を追加し、`--sign` フラグで HMAC (`--sig-mode hmac`) または Ed25519+HMAC dual (`--sig-mode dual`) のいずれかで Echo Mark を生成し、出力 JSON の `echo_mark` フィールドに格納。`--key-id` / `--keys-dir` / `--run-id` も追加し、既存の badge / voice コマンドと一貫した署名 CLI 規約を維持。
- `tests/test_device_cli.py` に HMAC 署名成功 / BLOCKED ラベル確認 / ECHO_CHECK ラベル確認 / reasons 監査追跡性 / secret 未設定 exit 1 の計5テストを追加。
- `tests/test_prop_device_boundary.py` に `to_audit_payload()` の必須フィールド契約 / `execution_allowed` 一貫性 / `reasons` 非空 / meta デフォルト / `build_payload` 互換性の計5テスト（うち3テストは Hypothesis property-based）を追加。

### v1.1.0 Sprint-2 判定

| 判定項目 | 判定 | 根拠 |
|---|---|---|
| 不変原則適合 | ✅ | デバイス決定の candidates/evidence/責任境界が Echo Mark 署名で監査可能化 |
| 非破壊性 | ✅ | 既存 `decide_for_device` / `to_responsibility_boundary` / CLI 動作を維持 |
| property-based test | ✅ | `to_audit_payload()` の契約を Hypothesis で自動検証 |
| 署名 CLI 規約の一貫性 | ✅ | badge / voice コマンドと同じ `--sig-mode` / `--key-id` 体系 |
| テスト健全性 | ✅ | 351テスト全パス（341既存 + 10新規） |


## ECHO-20260412-002 v1.1.0 Sprint-3 トレーサビリティ & CI カバレッジ強化

### Sprint-3 バックログ

| 優先度 | タスク | 目的 | 完了条件 | ステータス |
|---|---|---|---|---|
| High | `traceability_v1.yaml` に `REQ-DEV-004` 追加 | Sprint-2 のデバイス署名要件を要件トレーサビリティに反映 | `to_audit_payload()` / `cmd_device` を code_refs として登録し、`validate_traceability_file` が ok=True を返す | ✅ |
| Mid | `test_traceability_validator.py` を CI smoke gate の Unit tests に追加 | トレーサビリティリンクの整合性（コード参照・テスト参照）をCIで自動検証 | `ci.yml` Unit tests ステップに追加し、リンク切れを PR 段階で検知可能化 | ✅ |
| Mid | `test_device_cli.py` を CI smoke gate の Unit tests に追加 | デバイス CLI 回帰を smoke 段階でも早期検知（kpi-quick と二重ゲート） | `ci.yml` Unit tests ステップに追加 | ✅ |

### 実装詳細

- `docs/traceability/traceability_v1.yaml` に `REQ-DEV-004` を追加し、`DeviceBoundaryDecision.to_audit_payload()` と `po-cosmic device --sign` の要件を明文化。`tests/test_prop_device_boundary.py::test_prop_audit_payload_has_required_fields` / `test_audit_payload_is_make_echo_mark_compatible` と `tests/test_device_cli.py::test_device_sign_hmac_produces_echo_mark` を証跡テストとして紐付け。
- `.github/workflows/ci.yml` の `smoke` ジョブ Unit tests ステップに `tests/test_device_cli.py` と `tests/test_traceability_validator.py` を追加。デバイス CLI 契約の回帰検知とトレーサビリティ整合性チェックが PR 段階で自動実行されるよう強化。
- `traceability_v1.yaml` の `last_updated` を `2026-04-12` に更新し、Sprint-2 以降の変更を反映。

### v1.1.0 Sprint-3 判定

| 判定項目 | 判定 | 根拠 |
|---|---|---|
| 不変原則適合 | ✅ | REQ-DEV-004 により署名要件が要件レベルで機械的に追跡可能化 |
| 非破壊性 | ✅ | 既存テスト・CI ジョブ・トレーサビリティ エントリを未改変 |
| トレーサビリティ整合性 | ✅ | `validate_traceability_file` が 17 エントリ全件 ok=True |
| CI カバレッジ | ✅ | smoke gate unit tests に device CLI + traceability validator を追加 |
| テスト健全性 | ✅ | 351テスト全パス（変更なし） |


## ECHO-20260412-003 v1.1.0 Sprint-4 デバイスレシート週次アーカイブ統合

### Sprint-4 バックログ

| 優先度 | タスク | 目的 | 完了条件 | ステータス |
|---|---|---|---|---|
| High | `WeeklyArchivePaths` に `device_receipt` フィールド追加 | デバイス署名レシートを週次監査証跡に組み込む | `audit_archive.py` を更新し `device_receipt.json` パスを追加、全関連テストがパス | ✅ |
| Mid | `weekly_audit_archive.py` でデバイスレシート自動生成 | 週次実行時に `po-cosmic device --sign` を実行して監査ループを閉じる | smart_speaker/booking シナリオのレシートを `device_receipt.json` に保存し outcomes に追記 | ✅ |
| Mid | `test_prop_audit_archive.py` に `device_receipt` フィールド契約テストを追加 | archive 命名契約を property-based test で回帰検知 | Hypothesis で `device_receipt.name == "device_receipt.json"` を全週 IDで検証 | ✅ |

### 実装詳細

- `src/po_echo/audit_archive.py` の `WeeklyArchivePaths` dataclass に `device_receipt: Path` フィールドを追加し、`ensure_weekly_archive` が `device_receipt.json` パスを返すよう更新。
- `scripts/weekly_audit_archive.py` に `device_receipt` 生成ステップを追加。`po-cosmic device --device smart_speaker --intent booking --out <path>` を実行し、`--hmac-secret` が指定されている場合は `--sign --sig-mode hmac` で署名付きレシートを生成。rc=3（実行ブロック）は正常な監査結果として `PASS` に分類し、それ以外の非ゼロのみ `FAIL` とする設計を明示。
- `tests/test_prop_audit_archive.py` の `test_ensure_weekly_archive_uses_fixed_filename_contract` に `archive.device_receipt.name == "device_receipt.json"` のアサーションを追加。

### v1.1.0 Sprint-4 判定

| 判定項目 | 判定 | 根拠 |
|---|---|---|
| 不変原則適合 | ✅ | デバイスレシートが週次証跡として候補/証拠/責任境界の監査ループに統合 |
| 非破壊性 | ✅ | 既存 `WeeklyArchivePaths` フィールド・スクリプトステップを未改変 |
| property-based test | ✅ | `device_receipt` 命名契約を Hypothesis で全週 ID に対して自動検証 |
| 監査ループ完結 | ✅ | Sprint-2 の署名実装 → Sprint-3 のトレーサビリティ → Sprint-4 のアーカイブで閉じた |
| テスト健全性 | ✅ | 351テスト全パス（変更なし） |
