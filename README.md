...
- 週次/月次アーカイブを横断する統合サマリーの自動生成
- KPI劣化検知のしきい値アラート雛形を運用導線へ追加
- Gift rehearsal履歴インデックスの自動更新

### 🚧 P2 Sprint-4 次タスク

- 統合サマリーの差分比較（前回実行との差分）を定型出力化
- KPIアラートテンプレートの記入チェックを軽量CLIで自動検証
- 異常検知（`has_reported_failures` / `has_malformed_artifact`）を週次運用で自動通知
...
- 🚀 **v0.5.0**（進行中）: P2運用定着（監査自動化・KPI継続監視・配布標準化）
- ⏳ **v1.0.0**（将来）: 公開監査運用を含む本番成熟
