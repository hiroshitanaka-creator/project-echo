...
**最終更新**: 2026-03-01  
**目的**: Grokコンテキスト制限対策用マスター参照シート（Phase 1最終調整版）。  
...
- **Phase 1 (3/1〜3/7)**: Core Defense 鉄壁化 ← 現在進行中
  - ECHO-20260301-001〜004: Semantic Diversity（6D tensor統合 + 回帰修正）
...
- **その他**:
  ...
  - CHANGELOG.md / PROGRESS.mdは毎タスク更新

#### 4. タスク命名・PRルール
- 形式: `ECHO-YYYYMMDD-XXX`（例: ECHO-20260301-004）
...
- 総合点7.0未満は即リジェクト
- Po_core APIとの乖離ゼロ
- semanticが「本物」（6D cosine tensor）になっていない場合は即修正
- エッジケース・パフォーマンス・セキュリティを必ず指摘
- 哲学的整合性もチェック

#### 6. 実装強化の優先順位（v1.1追加）
- **非破壊を最優先**: 既存Doberman/gate_audioフローは変更しない。
- **弱点のみ強化**: semantic証拠の精度・再現性・監査性を上げる。
- **証拠を残す**: すべての強化は `semantic_delta`, `6d_values`, `freedom_pressure_snapshot` に反映。
- **回帰禁止**: 10k候補規模でもクラッシュしない。

#### 7. 運用ルール
- Codex出力は**全文diffパッチでGrokに貼る**
- Bibleは10〜15メッセージに1回貼る
- Phase完了ごとにBibleを更新（v1.2, v1.3 ...）
