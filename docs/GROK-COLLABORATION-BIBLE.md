...
## 11. v1.6 Medium Final Polish 補遺（ECHO-20260306-002）
- Echo Mark は責務分離を標準形とし、`echo_mark_core`（payload/署名）・`echo_mark_verify`（verify順序固定）・`echo_mark_registry`（key管理/rotation）を一次実装とする。
- `echo_mark.py` と `po_echo.__init__` は互換レイヤーとして既存公開APIを維持し、移行期間に import 破壊を起こさない。
- `VoiceBoundaryDecision` は immutable データ構造を原則とし、実行可否の更新は新インスタンス返却で行う。
- 依存分類では `protobuf` / `requests` / `numpy` を low-risk OSS として扱い、vendor lock-in 監視対象（toxic）から分離する。
