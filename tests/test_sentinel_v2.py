...
from po_echo.sentinel_v2 import apply_semantic_diversity, scan_directory, scan_directory_optimized
...
def test_scan_directory_optimized_supports_exclude_glob(
    tmp_path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Excluded files must not be counted/scanned."""
...
def test_scan_directory_optimized_supports_changed_files_filter(
    tmp_path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Only changed-files list entries should be scanned."""
    bad = tmp_path / "broken.py"
    ok = tmp_path / "ok.py"
    bad.write_text("def broken(:\n", encoding="utf-8")
    ok.write_text("x = 1\n", encoding="utf-8")
